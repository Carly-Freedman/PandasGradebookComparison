...
numDataEl = 0
roster = pd.read_csv(
    DATA_FOLDER / "roster1.csv",
    converters={"NetID": str.lower, "Email Address": str.lower},
    usecols=["Section", "Email Address", "NetID"],
    index_col="NetID",
)
numDataEl += len(roster) * len(roster.columns)
...
numDataEl += len(hw_exam_grades) * len(hw_exam_grades.columns)
...
numDataEl += len(quiz_grades) * len(quiz_grades.columns)
# ------------------------
